...
def generate_horse_comment(horse_name, features, top5_patterns, bottom_patterns, sex=None):
    """個別馬のコメントを生成（個性的・エンタメ表現・性別対応）

    sex: '牝'(牝馬)/'牡'(牡馬)/'セ'(セン馬)/None(不明)
    各フレーズに性別タグ('N'=中立/'F'=牝専用/'M'=牡専用/'S'=セン馬専用)を付与し、
    対応する性別 + 中立のフレーズだけ選択する。
    """
    import random
    if not features:
    ...
    EYE_PHRASES = {
        '細長い': [
            ("ギラリと光る細い眼光、獲物を狙うタカの目", 'N'),
            ("鋭く細長い目が語りかける『勝ちに来た』感", 'N'),
            ("細目から漏れ出る殺気、レース前のオーラが違う", 'N'),
            ("研ぎ澄まされた針のような目つき、内に秘めた獰猛さ", 'N'),
            ("細く鋭い眼差し、修練を積んだ侍のような気迫", 'N'),
        ],
        '切れ長': [
            ("スッと引いた切れ長の目、クールビューティーの極み", 'F'),
            ("妖艶な切れ長の眼差し、惚れ惚れする美しさ", 'F'),
            ("優雅な切れ長眼、舞台女優のような佇まい", 'F'),
            ("クールでシャープな眼差し、冷静沈着な勝負師", 'N'),
            ("二重の切れ長、ハリウッドスター級の佇まい", 'M'),
            ("武士のような凛とした切れ長眼差し", 'M'),
            ("涼やかな切れ長の目、知性が滲み出る顔つき", 'N'),
        ],
        '丸い': [
            ("クリクリとした丸い目、見た目はおとなしそうだが…", 'N'),
            ("丸くてつぶらな瞳、でも内には闘志を秘めているはず", 'N'),
            ("つぶらな瞳が愛嬌たっぷり、ファン心をくすぐる", 'N'),
            ("丸目に宿る無垢な眼差し、純粋ゆえの強さ", 'N'),
            ("くりっとした丸目、子鹿のような愛らしさ", 'N'),
        ],
        'アーモンド型': [
            ("アーモンド型の端正な目元、育ちの良さがにじみ出る", 'N'),
            ("均整のとれたアーモンド眼、王道サラブレッドの風格", 'N'),
            ("整ったアーモンド型の瞳、品格を感じさせる名門の血筋", 'N'),
            ("バランス絶妙のアーモンド眼、優等生タイプの顔立ち", 'N'),
            ("理想的なアーモンド眼、教科書のような美形", 'N'),
        ],
    }
    NOSTRIL_PHRASES = {
        '大きい': [
            ("鼻孔が豪快に開き、エンジン全開の予感", 'N'),
            ("『吸気量が違う！』と叫びたくなるほど大きな鼻孔", 'N'),
            ("ターボエンジン級の大鼻孔、酸素を貪るスタミナ型", 'N'),
            ("豪快に開いた鼻、息遣いから戦闘モードが伝わる", 'N'),
            ("掃除機級の吸気力、走る空気清浄機", 'N'),
        ],
        '小さい': [
            ("すっきりとした小ぶりな鼻孔、洗練されたフォルム", 'N'),
            ("小鼻がコンパクトで女優級の端正さ", 'F'),
            ("小鼻がシャープで二枚目俳優のような彫りの深さ", 'M'),
            ("小ぶりな鼻、効率重視のスプリント特化型", 'N'),
            ("ピンと締まった小鼻、無駄を削ぎ落とした機能美", 'N'),
        ],
        '中程度': [
            ("鼻孔は標準的でバランス重視タイプ", 'N'),
            ("鼻はオーソドックス、可もなく不可もなし", 'N'),
            ("ごく普通の鼻、安定の優等生タイプ", 'N'),
        ],
    }
    JAW_PHRASES = {
        '強い': [
            ("顎がゴツゴツとした筋肉質、噛み合わせ最強感", 'N'),
            ("『ここぞ』で食らいつく力強い顎、競り合いに強そう", 'N'),
            ("武骨で頼もしいエラ張り、ガチンコ勝負の主役タイプ", 'M'),
            ("発達した顎の筋肉、底力で押し切る粘り強さ", 'N'),
            ("骨太な顎、雌豹のような野性味", 'F'),
            ("セン馬らしい安定した顎ライン、闘争心は失わず", 'S'),
        ],
        '弱い': [
            ("顎がシュッとシャープ、スピード特化型かも", 'N'),
            ("顎の線は細め、スタミナより瞬発力勝負のタイプ", 'N'),
            ("繊細な顎ライン、儚げな美しさを纏う", 'F'),
            ("シャープな顎、貴公子然としたエレガント系", 'M'),
            ("線の細い顎、繊細さが武器になる玄人好み", 'N'),
        ],
        '中程度': [
            ("顎のバランスは標準的、オールラウンダー体型", 'N'),
            ("顎は中程度、特別な武器もないが弱点もない", 'N'),
            ("過不足ない顎、優等生らしい平均値", 'N'),
        ],
    }
    IMPRESSION_PHRASES = {
        '威圧感がある': [
            ("パドックに登場した瞬間、他馬がざわめく威圧感", 'N'),
            ("圧倒的な貫禄、近寄りがたいほどの存在感を放つ", 'N'),
            ("『この子が主役よ』と全身で語る女王のオーラ", 'F'),
            ("百獣の王のような風格、王者の眼光を備えた牡馬", 'M'),
            ("達観したセン馬の凄み、すべてを見下ろす佇まい", 'S'),
            ("近づくのも憚られる迫力、闘魂の塊", 'N'),
            ("威風堂々、出走前から勝者の風格を漂わせる", 'N'),
        ],
        '精悍': [
            ("精悍な顔つき、百戦錬磨の勝負師の表情", 'N'),
            ("キレッキレの精悍フェイス、見るからに勝負強そう", 'N'),
            ("凛とした精悍さは一流競走馬の証", 'N'),
            ("精悍な美貌、まさに男前という言葉が似合う牡馬", 'M'),
            ("精悍な中に上品さが漂う凛々しい牝馬", 'F'),
            ("研ぎ澄まされた精悍な表情、勝つために生まれた顔", 'N'),
            ("ナイフのように鋭く精悍、ピンッと張りつめた緊張感", 'N'),
        ],
        '温和': [
            ("温和な表情でパドックを周回、でも本番は別人に化けるタイプかも", 'N'),
            ("見た目はおっとり系…『おしとやかな刺客』の可能性", 'F'),
            ("穏やかな笑顔の裏に闘志を秘めた大物感", 'N'),
            ("癒し系の佇まい、勝つときは静かに圧倒的に勝つタイプ", 'N'),
            ("温和な紳士然とした顔、油断厳禁の隠れた実力派", 'M'),
            ("優しげな表情に騙されるな、本性は獰猛な野生馬", 'N'),
        ],
        '穏やか': [
            ("穏やかでリラックスした面構え、本番に強い精神力の証", 'N'),
            ("落ち着きすぎてて逆に怖い、鈍感力で勝ちに行くタイプ", 'N'),
            ("マイペースを貫く肝の据わった女傑の風格", 'F'),
            ("マイペースな貴公子、自分のリズムを崩さない強さ", 'M'),
            ("セン馬らしい穏やかさ、無心で走るのが強み", 'S'),
            ("禅僧のような穏やかさ、勝敗にすら執着しない無欲の境地", 'N'),
        ],
        '神経質': [
            ("神経質そうな目つき、集中力MAX・良い意味でピリピリ", 'N'),
            ("鋭く神経質な顔つき、ゲートが開いた瞬間に爆発する予感", 'N'),
            ("繊細なセンサーが全開、コンディション次第で大仕事をやらかす", 'N'),
            ("繊細な気性、ハマった時の爆発力は底知れない", 'N'),
            ("ピリピリ感が漲る、研ぎ澄まされた野生", 'N'),
            ("神経質ゆえの繊細さ、職人気質のレーサー", 'N'),
        ],
        '落ち着いている': [
            ("どっしり泰然、コーナーでごちゃごちゃしても動じない", 'N'),
            ("ベテランの風格漂う落ち着き、レースで慌てない鉄のメンタル", 'N'),
            ("どんな展開でも顔色ひとつ変えない究極の平常心", 'N'),
            ("セン馬らしい落ち着き、淡々と仕事をこなすプロ", 'S'),
            ("どっしりと構える女王然とした余裕", 'F'),
            ("動じない貫禄、王者の風格を纏う牡馬", 'M'),
            ("湖面のように静かな佇まい、嵐の中でも冷静沈着", 'N'),
        ],
    }
    CONTOUR_PHRASES = {
        '丸顔': [
            ("ふっくら丸顔は愛嬌たっぷり、ファン投票なら1位かも", 'N'),
            ("まんまるな顔、このかわいさが逆に盲点になりそう", 'N'),
            ("愛くるしい丸顔に隠れた闘志、油断したら置いていかれる", 'N'),
            ("丸顔キュートな貴婦人、可憐さの裏に勝負根性", 'F'),
            ("丸顔で愛嬌たっぷりの童顔系、若さで押し切るタイプ", 'N'),
            ("マシュマロのような優しい丸顔、本性は侮れない", 'N'),
        ],
        '面長': [
            ("すらりとした面長、サラブレッドの理想形を体現", 'N'),
            ("面長で鼻筋がスッと通り、一流感ただよう顔立ち", 'N'),
            ("モデル顔負けの面長美人、走りも姿も一流志向", 'F'),
            ("彫りの深い面長フェイス、ハリウッドスター級の男前", 'M'),
            ("理知的な面長、すべて計算ずくで走る冷静派", 'N'),
            ("貴族的な面長、サラブレッド界のエリート", 'N'),
        ],
        '逆三角形': [
            ("逆三角形の輪郭は強者の証、ライバルをにらみ倒す", 'N'),
            ("逆三角フェイスに宿る闘争心、直線で弾けるタイプ", 'N'),
            ("キリッとした逆三角の輪郭、強い馬に多いシャープな顔型", 'N'),
            ("逆三角のシュッとしたフォルム、洗練された美馬", 'N'),
            ("逆三角の鋭さ、剣士のような切れ味", 'N'),
        ],
        '正方形': [
            ("四角くどっしりした顔、パワーで押し切るタイプ", 'N'),
            ("がっちりした四角顔、揉み合いも辞さないタフさが売り", 'N'),
            ("骨格がしっかりした四角顔、スタミナ勝負に強そう", 'N'),
            ("武骨な四角顔、漢らしさ満点の本格派", 'M'),
            ("セン馬の四角顔、力強さと安定感を両立", 'S'),
            ("ボクサーのような四角顔、タフネス指数MAX", 'N'),
        ],
    }
    FOREHEAD_PHRASES = {
        '広い': [
            ("広い額はIQ高そう、頭脳派レースに強い予感", 'N'),
            ("広大な額に『勝利への地図』が描かれているかも", 'N'),
            ("広い額に知性が宿る、戦略派の名馬の顔", 'N'),
            ("広々とした額、思考の深さが伝わる賢者の顔", 'N'),
        ],
        '狭い': [
            ("キュッと引き締まった額、余計なことを考えない一点突破型", 'N'),
            ("狭い額はシャープな集中力の象徴、直線勝負に賭ける", 'N'),
            ("狭い額、純粋なスピード型のシンプル思考", 'N'),
        ],
        '中程度': [
            ("額は標準的、特筆なし", 'N'),
            ("額のバランスは普通、可もなく不可もなし", 'N'),
        ],
    }
    ...
    def pick(d, key):
        opts = d.get(key, [])
        filtered = [t for (t, tag) in opts if tag in allowed_tags]
...
def main():
    ...
    for row in entry_rows:
        horse_name, horse_id, image_path, _race_id = row[0], row[1], row[2], row[3]
        horse_sex = row[4] if len(row) > 4 else None
        print(f"  {horse_name} (id={horse_id}{'/' + horse_sex if horse_sex else ''})")
        ...
        # 詳細コメント生成（4次元）
        face_comment = generate_horse_comment(horse_name, features, top5_patterns, bottom_patterns, sex=horse_sex)
