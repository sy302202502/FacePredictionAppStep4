...
def record_result(race_name, actual_winner_name):
    ...
    try:
        ...
        try:
            ...
            predicted_winner = predictions[0][1]
            ...
            for pred_id, horse_name, pred_rank, final_score, cat, r_date in predictions:
                ...
                # top5_hit は予測上位5頭の行にのみ記録
                row_top5 = top5_hit if pred_rank is not None and pred_rank <= 5 else None
                ...
                if existing:
                    ...
                    """, (actual_rank, hit_1st and pred_rank == 1, row_top5, pred_id))
                else:
                    cur.execute("""
                    ...
                    """, (
                        ...
                        hit_1st and pred_rank == 1, row_top5, final_score
                    ))
...
def show_report(by_category=False):
    ...
    try:
        ...
        print("\n  --- 直近10レースの予想精度 ---")
        cur.execute("""
            SELECT race_name, race_date, race_category, top5_hit
            FROM (
                SELECT DISTINCT ON (race_name)
                    race_name, race_date, race_category, top5_hit
                FROM prediction_accuracy
        ...
        """)
        recent = cur.fetchall()
        for rname, rdate, cat, t5 in recent:
            mark = "✓" if t5 else "✗"
